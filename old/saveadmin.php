		...
		if ($file !== "." &&  $file !== ".gitignore" && $file !== ".."  && file_exists("data/user/" . $file))
		{
			...
			foreach($matchdata as $group)
			{
				foreach ($group["matches"] as $match)
				{
					...
					$userdata["points"][str_replace(" ",  "_",$group["naam"]).'_'.str_replace(" ",  "_",$match['thuis']).'_' . str_replace(" ",  "_",$match['uit'])] = $punten;
			...
			if(isset($_POST["antwoorden"]))
				foreach($_POST["antwoorden"] as $user => $vraag)
					if($user == $userdata["username"])
						...
						if($userdata["extra"] == true)
							$scoreboard[$userdata["name"]]["extra"] = true;
